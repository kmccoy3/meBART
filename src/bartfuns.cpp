...
void makexinfo(size_t p, size_t n, double *x, xinfo &xi, int *nc)
{
    ...
    for (size_t i = 0; i < p; i++)
    {
        ...
        xi[i].resize(nc[i]);
        for (size_t j = 0; j < (size_t)nc[i]; j++)
...
double getpb(tree &t, xinfo &xi, pinfo &pi, tree::npv &goodbots)
{
    ...
    if (goodbots.size() == 0)
    { // are there any bottom nodes you can split on?
        pb = 0.0;
    }
    else
    {
        if (t.treesize() == 1)
            pb = 1.0; // is there just one node?
        else
            pb = pi.pb;
    }
    return pb;
...
void bprop(tree &x, xinfo &xi, pinfo &pi, tree::npv &goodbots, double &PBx, tree::tree_p &nx, size_t &v, size_t &c, double &pr, std::vector<size_t> &nv, std::vector<double> &pv, bool aug, rn &gen)
{
    ...
    else
    {
        ...
        if (nbadvars != 0)
        { // if we have bad vars then we need to augment, otherwise we skip
...
void dprop(tree &x, xinfo &xi, pinfo &pi, tree::npv &goodbots, double &PBx, tree::tree_p &nx, double &pr, rn &gen)
{
    ...
    if (nx->ntype() == 't')
    { // is the nog node nx the top node
        PBy = 1.0;
    }
    else
    {
    ...
    if (cansplit(nx->getl(), xi))
        --ngood; // if can split at left child, lose this one
    if (cansplit(nx->getr(), xi))
        --ngood; // if can split at right child, lose this one
...
void draw_s(std::vector<size_t> &nv, std::vector<double> &lpv, double &theta, rn &gen)
{
    ...
    for (size_t j = 0; j < p; j++)
        _theta[j] = theta / (double)p + (double)nv[j];
