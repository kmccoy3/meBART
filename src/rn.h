...
public:
    // constructor
    arn() {}
    arn(unsigned int n1, unsigned int n2)
    {
        this->n1 = n1;
        this->n2 = n2;
    }
    ...
    {
        if (shape < 0.01)
            return ::exp(this->log_gamma(shape)) / rate;
        else
        {
            gam = gamD(shape, 1.);
            return ((gam)(gen)) / rate;
    ...
    virtual void set_wts(std::vector<double> &_wts)
    {
        double smw = 0.;
        wts.clear();
        for (size_t j = 0; j < _wts.size(); j++)
            smw += _wts[j];
        for (size_t j = 0; j < _wts.size(); j++)
            wts.push_back(_wts[j] / smw);
    }
